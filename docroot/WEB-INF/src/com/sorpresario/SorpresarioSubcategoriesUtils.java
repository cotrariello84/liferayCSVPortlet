...
public class SorpresarioSubcategoriesUtils {

	private static Log _log = LogFactoryUtil
			.getLog(SorpresarioCategoryImportUtils.class);

	public static JournalArticle getSubcategoryByName(ActionRequest actionRequest,String nome) {
	...
	public static boolean joinSubcategoryToFamily(ActionRequest actionRequest,	JournalArticle category, JournalArticle family) {

		ThemeDisplay themeDisplay = (ThemeDisplay) actionRequest.getAttribute(WebKeys.THEME_DISPLAY);

		try {
			AssetEntry assetEntryCategory = AssetEntryLocalServiceUtil.fetchEntry(JournalArticle.class.getName(),category.getResourcePrimKey());
			...
			if (existLink(assetEntryCategory.getEntryId(),assetEntryFamily.getEntryId())) {
			} else {
				AssetLinkLocalServiceUtil.addLink(themeDisplay.getUserId(),assetEntryCategory.getEntryId(),assetEntryFamily.getEntryId(), 0, 0);
			}
		} catch (SystemException e) {
			_log.error(e);
			return false;
	...
	public static JournalArticle createSubcategory(ActionRequest actionRequest,String title, byte[] image) {
		...
		try {
			...
			String structureKey = SorpresarioUtils.getStructureKey("StyleGuide-Categories");
			String templateKey = SorpresarioUtils.getTemplateKey("TPL StyleGuide-Categories");

			String content = SorpresarioUtils.getXMLStructureContentCategory();
